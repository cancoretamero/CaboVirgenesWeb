...
import { 
  Menu, X, Anchor, ArrowRight, Play, Instagram, Linkedin, Facebook, 
  MapPin, Thermometer, Award, Globe, Ship, Users, CheckCircle, 
  ChevronDown, Leaf, Calendar, Mail, Phone, Send, Search, Clock, 
  Droplets, ShieldCheck, Activity, Navigation, Wind, Fuel, Gauge,
  Fish, Box, Layers, ExternalLink, MessageCircle, Pause, Maximize2, 
  Volume2, VolumeX, Settings, Captions, VideoOff
} from 'lucide-react';
...
const CaboVirgenesHome = () => {
  ...
  const NEWS_DATA = [
    {
      ...
      fecha: "13/05/2025",
      titulo: "El mercado y el consumidor son muy dinámicos, tenemos que adaptarnos",
  ...
  // Datos del submenú (ACTUALIZADOS SEGÚN PETICIÓN)
  const NAV_ITEMS = [
    { 
      label: 'Quienes Somos', 
      id: 'quienes_somos',
  ...
  return (
    <div className="min-h-screen bg-[#0f172a] text-white font-['Poppins'] selection:bg-cyan-500 selection:text-white overflow-x-hidden">
      ...
      {/* --- LIQUID GLASS NAVBAR --- */}
      <nav 
        className={`fixed top-6 left-1/2 -translate-x-1/2 z-50 w-[95%] max-w-6xl transition-all duration-500 ${isScrolled ? 'top-4 scale-95' : 'top-8'}`}
        onMouseLeave={() => setActiveSubmenu(null)}
      >
        <div className={`glass-nav rounded-3xl px-6 py-4 transition-all duration-300 relative ${isScrolled ? 'bg-[#0f172a]/90' : ''}`}>
          <div className="flex items-center justify-between gap-8">
            <div className="flex items-center gap-3">
              <div className="bg-cyan-500/20 p-2 rounded-full">
                <Anchor className="text-cyan-400 w-5 h-5" />
              </div>
              <span className="font-medium tracking-[0.2em] text-sm uppercase text-white/90">Cabo Vírgenes</span>
            </div>
            ...
            <div className="hidden md:flex items-center gap-10 text-xs font-medium tracking-widest uppercase text-slate-300">
              {NAV_ITEMS.map((item) => (
                <div 
                  ...
                  onMouseEnter={() => item.subitems.length > 0 ? setActiveSubmenu(item.id) : setActiveSubmenu(null)}
                >
                  <a 
                    href={`#${item.id}`}
                    className={`hover:text-white transition-all flex items-center gap-1 ${activeSubmenu === item.id ? 'text-white' : ''}`}
                  >
                    {item.label}
                    {item.subitems.length > 0 && (
                      <ChevronDown className={`w-3 h-3 transition-transform duration-300 ${activeSubmenu === item.id ? 'rotate-180 text-cyan-400' : ''}`} />
                    )}
                  </a>
                  <span className={`absolute bottom-0 left-1/2 -translate-x-1/2 h-[2px] bg-cyan-400 transition-all duration-300 ${activeSubmenu === item.id ? 'w-full' : 'w-0 group-hover:w-full'}`}></span>
                </div>
              ))}
            </div>
          ...
          <div className={`absolute top-full left-0 w-full pt-4 transition-all duration-300 origin-top ${activeSubmenu ? 'opacity-100 translate-y-0 visible' : 'opacity-0 -translate-y-4 invisible'}`}>
            <div className="glass-submenu rounded-2xl p-8 grid grid-cols-12 gap-8">
               <div className="col-span-4 border-r border-white/10 pr-6">
                 <h3 className="text-2xl font-light text-white mb-2">{NAV_ITEMS.find(i => i.id === activeSubmenu)?.label}</h3>
                 <p className="text-slate-400 text-xs leading-relaxed">
                   {activeSubmenu === 'quienes_somos' && "Tradición pesquera e infraestructura global de primer nivel."}
                   {activeSubmenu === 'productos' && "Excelencia en mariscos salvajes y procesados del Atlántico Sur."}
                 </p>
               ...
               <div className="col-span-8 grid grid-cols-2 gap-4">
                 {NAV_ITEMS.find(i => i.id === activeSubmenu)?.subitems.map((sub, idx) => (
                   <a key={idx} href="#" className="flex items-start gap-3 p-3 rounded-xl hover:bg-white/5 transition-colors group">
                     <div className="mt-1 w-1.5 h-1.5 rounded-full bg-cyan-500 group-hover:shadow-[0_0_10px_#06b6d4] transition-all"></div>
                     <div>
                       <div className="text-sm font-medium text-slate-200 group-hover:text-white transition-colors">{sub.title}</div>
      ...
      <header className="relative min-h-[110vh] w-full flex items-center justify-center overflow-hidden pb-32">
        <div className="absolute inset-0 z-0">
          <img src="https://images.unsplash.com/photo-1498623116890-37e912163d5d?q=80&w=2576&auto=format&fit=crop" alt="Ocean Waves" className="w-full h-full object-cover opacity-50 scale-105 animate-pulse-slow" />
        ...
        <div className="relative z-10 text-center px-4 max-w-6xl mx-auto pt-20 animate-float">
          <div className="inline-flex items-center gap-3 border border-white/10 bg-white/5 backdrop-blur-md rounded-full px-5 py-2 mb-8">
            ...
            <span className="text-cyan-100/80 text-[11px] font-medium tracking-[0.2em] uppercase">Premium Wild Catch</span>
          </div>
          <h1 className="text-6xl md:text-9xl font-extralight tracking-tighter mb-8 leading-[0.9] text-glow mix-blend-overlay opacity-90">
            MAR <br/> <span className="font-semibold italic text-white opacity-100 mix-blend-normal">PROFUNDO</span>
          </h1>
          <p className="text-slate-200 text-lg md:text-2xl font-light max-w-2xl mx-auto mb-12 leading-relaxed tracking-wide opacity-90">
            Langostino Salvaje Patagónico. <br/> <span className="text-cyan-200/60 text-base">Capturado en origen. Congelado a bordo.</span>
          </p>
          <div className="flex flex-col sm:flex-row gap-6 justify-center items-center">
            <button className="group relative px-10 py-5 bg-white text-slate-900 rounded-full font-semibold tracking-wide overflow-hidden hover:scale-105 transition-all duration-300 shadow-[0_0_40px_-10px_rgba(255,255,255,0.3)]">
              <span className="relative z-10 flex items-center gap-2">Explorar Colección <ArrowRight className="w-4 h-4 group-hover:translate-x-1 transition-transform" /></span>
            </button>
            <button 
            ...
            >
              <div className="w-8 h-8 rounded-full border border-white/30 flex items-center justify-center group-hover:border-cyan-400 group-hover:bg-cyan-400/10 transition-all"><Play className="w-2.5 h-2.5 ml-0.5 fill-current" /></div> Ver Video
            </button>
          </div>
        </div>
      </header>
      ...
      <div className="relative w-full -mt-48 z-20 pointer-events-none">
        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1440 320" className="w-full h-auto block align-middle">
          <path fill="#0f172a" fillOpacity="1" d="M0,224L80,213.3C160,203,320,181,480,181.3C640,181,800,203,960,224C1120,245,1280,267,1360,277.3L1440,288L1440,320L1360,320C1280,320,1120,320,960,320C800,320,640,320,480,320C320,320,160,320,80,320L0,320Z"></path>
        </svg>
      </div>
        ...
        <div className="container mx-auto px-6">
          <div className="flex flex-col md:flex-row items-end justify-between mb-20 gap-8">
            <div className="max-w-2xl">
              <span className="text-cyan-400 font-bold tracking-[0.2em] text-xs uppercase mb-4 flex items-center gap-2"><span className="w-8 h-[1px] bg-cyan-400"></span> Origen</span>
              <h2 className="text-4xl md:text-6xl font-light leading-tight text-white">De la Patagonia <br /> <span className="font-serif italic text-slate-500">al mundo.</span></h2>
            </div>
            <p className="text-slate-400 max-w-md text-sm leading-relaxed font-light border-l border-slate-700 pl-6">Operamos en la Zona FAO 41, un santuario natural donde las corrientes frías crean el hábitat perfecto para el langostino rojo salvaje.</p>
          </div>
          <div className="grid lg:grid-cols-12 gap-8 items-center">
            <div className="lg:col-span-7 relative group">
              <div className="relative h-[600px] rounded-[3rem] overflow-hidden shadow-2xl">
                <img src="https://images.unsplash.com/photo-1589191995092-668887805631?q=80&w=1740&auto=format&fit=crop&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D" alt="Barco Pesquero" className="w-full h-full object-cover transform group-hover:scale-110 transition-transform duration-1000" />
                ...
                <div className="space-y-8">
                  {[
                    ...
                    <div key={i} className="flex gap-6 items-start">
                      <div className="p-3 bg-white/5 rounded-2xl border border-white/10"><item.icon className="w-6 h-6 text-cyan-400" /></div>
                      <div><h4 className="text-lg font-medium mb-1">{item.title}</h4><p className="text-sm text-slate-400 font-light">{item.text}</p></div>
                    </div>
                  ))}
                </div>
              </div>
      ...
      <div className="relative w-full -mb-1 z-10 pointer-events-none bg-slate-50">
        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1440 320" className="w-full h-auto block rotate-180 fill-[#0f172a]">
      ...
      <section className="py-32 bg-slate-50 relative overflow-hidden">
        <div className="container mx-auto px-6 relative z-10">
           <div className="text-center mb-24">
             <span className="text-cyan-600 font-bold tracking-[0.2em] text-xs uppercase mb-4 inline-block px-4 py-2 rounded-full bg-cyan-50 border border-cyan-200">Integración Vertical</span>
             <h2 className="text-4xl md:text-5xl font-light text-[#0f172a] mb-6">Cadena de Valor</h2>
             <p className="text-slate-600 font-light max-w-2xl mx-auto text-lg">Controlamos cada etapa del proceso para garantizar una calidad sin compromisos.</p>
           </div>
              ...
              {[
                ...
                <div key={i} className="relative group text-center">
                  <div className="w-24 h-24 mx-auto bg-white border border-slate-200 rounded-full flex items-center justify-center relative z-10 group-hover:border-cyan-500 transition-colors duration-500 mb-8 shadow-xl shadow-slate-200">
                    ...
                    <div className="absolute -top-3 -right-3 w-8 h-8 bg-[#0f172a] text-white font-bold rounded-full flex items-center justify-center text-xs shadow-lg">{item.step}</div>
                  </div>
      ...
      <div className="relative w-full -mt-1 z-10 pointer-events-none bg-[#0a1120]">
         <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1440 320" className="w-full h-auto block fill-slate-50">
           <path fillOpacity="1" d="M0,96L48,112C96,128,192,160,288,160C384,160,480,128,576,122.7C672,117,768,139,864,154.7C960,171,1056,181,1152,165.3C1248,171,1344,149,1392,138.7L1440,128L1440,0L1392,0C1344,0,1248,0,1152,0C1056,0,960,0,864,0C768,0,672,0,576,0C480,0,384,0,288,0C192,0,96,0,48,0L0,0Z"></path>
         </svg>
      </div>
        ...
        <div className="container mx-auto px-6">
           <div className="flex flex-col md:flex-row justify-between items-end mb-16 gap-6">
              <div>
                ...
                <p className="text-slate-400 font-light">Tecnología naval al servicio de la frescura. Monitoreo en tiempo real.</p>
              </div>
              <div className="flex gap-2">
                 <span className="text-xs text-slate-500 uppercase tracking-widest mr-2 animate-pulse">Desliza para ver más</span>
           ...
           <div className="flex overflow-x-auto gap-6 pb-12 snap-x scrollbar-hide -mx-6 px-6">
             {FLEET_DATA.map((ship) => (
               ...
               >
                 <img src={ship.img} alt={ship.name} className="absolute inset-0 w-full h-full object-cover transition-transform duration-700 group-hover:scale-110" />
                 ...
                 <div className="absolute top-6 right-6">
                    <div className="bg-black/60 backdrop-blur-md px-3 py-1 rounded-full border border-white/10 flex items-center gap-2">
                       <div className={`w-2 h-2 rounded-full ${ship.status === 'Faenando' ? 'bg-green-500 animate-pulse' : 'bg-yellow-500'}`}></div>
                       <span className="text-[10px] text-white font-bold uppercase tracking-widest">{ship.status}</span>
                    </div>
                 </div>
                 ...
                 <div className="absolute bottom-0 left-0 p-8 w-full">
                   <div className="flex justify-between items-end mb-4 border-b border-white/20 pb-4">
                      <div>
                        <div className="text-cyan-400 text-[10px] font-bold uppercase tracking-widest mb-1">{ship.type}</div>
                   ...
                   <div className="flex justify-between items-center opacity-0 group-hover:opacity-100 transition-opacity transform translate-y-4 group-hover:translate-y-0 duration-300">
                     ...
                     <div className="w-8 h-8 rounded-full bg-cyan-500 flex items-center justify-center text-[#0f172a]"><Activity className="w-4 h-4" /></div>
                   </div>
                 </div>
               </div>
             ))}
      ...
      <section className="py-32 bg-[#0f172a] relative overflow-hidden">
        ...
        <div className="container mx-auto px-6 relative z-10 text-center">
           <span className="text-cyan-400 font-bold tracking-[0.2em] text-xs uppercase mb-6 block">Alcance Global</span>
           <h2 className="text-5xl font-light text-white mb-8">
             Cabo Vírgenes en <span className="font-serif italic text-cyan-200">Cifras</span>
           </h2>
           ...
           <div className="grid md:grid-cols-4 gap-6">
              {[
                { num: "75", label: "Países Exportados", icon: Globe, color: "text-cyan-400", bg: "from-cyan-500/20 to-blue-600/5", border: "border-cyan-500/30" },
                { num: "2.000", label: "Clientes que nos Avalan", icon: Users, color: "text-emerald-400", bg: "from-emerald-500/20 to-teal-600/5", border: "border-emerald-500/30" },
                { num: "100.000", label: "m³ Cámaras de Refrigerado", icon: Box, color: "text-blue-400", bg: "from-blue-500/20 to-indigo-600/5", border: "border-blue-500/30" },
                { num: "17.000", label: "Ton. Comercializadas / Año", icon: Ship, color: "text-teal-300", bg: "from-teal-400/20 to-cyan-600/5", border: "border-teal-400/30" }
              ].map((stat, i) => (
                <div key={i} className={`relative p-8 rounded-[2.5rem] border ${stat.border} bg-gradient-to-br ${stat.bg} backdrop-blur-md group hover:-translate-y-2 transition-all duration-500 hover:shadow-[0_0_40px_rgba(0,0,0,0.3)] overflow-hidden`}>
                   {/* Animated Background Blob */}
                   <div className={`absolute -right-10 -top-10 w-32 h-32 rounded-full blur-[60px] opacity-20 bg-current ${stat.color} group-hover:opacity-40 transition-opacity`}></div>
                   
                   <div className={`w-16 h-16 mx-auto rounded-full border border-white/10 bg-white/5 flex items-center justify-center mb-6 group-hover:scale-110 transition-transform duration-500 ${stat.color}`}>
      ...
      <div className="relative w-full -mb-1 z-10 pointer-events-none bg-[#0f172a]">
        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1440 320" className="w-full h-auto block">
          <path fill="#111c35" fillOpacity="1" d="M0,96L48,112C96,128,192,160,288,160C384,160,480,128,576,122.7C672,117,768,139,864,154.7C960,171,1056,181,1152,165.3C1248,149,1344,107,1392,85.3L1440,64L1440,320L1392,320C1344,320,1248,320,1152,320C1056,320,960,320,864,320C768,320,672,320,576,320C480,320,384,320,288,320C192,320,96,320,48,320L0,320Z"></path>
        </svg>
      </div>
        ...
        <div className="container mx-auto px-6 relative z-10">
          <div className="flex flex-col md:flex-row justify-between items-end mb-20 gap-6">
            <div className="max-w-3xl">
              <span className="text-cyan-400 font-bold tracking-[0.2em] text-xs uppercase mb-6 inline-block px-4 py-2 rounded-full bg-cyan-900/10 border border-cyan-500/20">Nuestra Selección</span>
              <h2 className="text-4xl md:text-5xl font-light text-white mb-6">Excelencia del Atlántico</h2>
              <p className="text-slate-400 font-light text-lg">Catálogo premium de mariscos salvajes y procesados.</p>
            </div>
            <div className="flex gap-2">
                 <span className="text-xs text-slate-500 uppercase tracking-widest mr-2 animate-pulse">Explorar Catálogo</span>
                 <ArrowRight className="w-4 h-4 text-cyan-400" />
            </div>
          </div>
          ...
          <div className="flex overflow-x-auto gap-6 pb-12 snap-x scrollbar-hide -mx-6 px-6">
            {PRODUCTS_DATA.map((item) => (
              ...
              >
                <img src={item.img} alt={item.title} className="absolute inset-0 w-full h-full object-cover transition-transform duration-[1.5s] ease-out group-hover:scale-110" />
                ...
                <div className="absolute inset-0 p-8 flex flex-col justify-end items-start">
                  <span className="inline-block px-3 py-1 bg-white/10 backdrop-blur-md border border-white/20 rounded-full text-[10px] font-bold tracking-widest uppercase text-cyan-300 mb-4 transform -translate-y-4 opacity-0 group-hover:translate-y-0 group-hover:opacity-100 transition-all duration-500">{item.tag}</span>
                  <h3 className="text-3xl font-light text-white mb-2 group-hover:text-cyan-200 transition-colors">{item.title}</h3>
                  <p className="text-slate-400 text-sm font-light mb-6 opacity-80 group-hover:opacity-100 max-w-[90%] line-clamp-2">{item.desc}</p>
                  <div className="w-12 h-12 rounded-full border border-white/30 flex items-center justify-center group-hover:bg-white group-hover:text-[#0f172a] transition-all duration-500"><ArrowRight className="w-5 h-5 -rotate-45 group-hover:rotate-0 transition-transform duration-500" /></div>
                </div>
              </div>
            ))}
          </div>
          ...
          <div className="mt-8 text-center">
            <button className="inline-flex items-center gap-3 text-sm font-light tracking-[0.2em] uppercase text-white hover:text-cyan-400 transition-colors border-b border-transparent hover:border-cyan-400 pb-1">Descargar Catálogo PDF Completo <ArrowRight className="w-4 h-4" /></button>
          </div>
        </div>
      </section>
      ...
      <section className="py-24 bg-[#111c35] border-t border-white/5">
        <div className="container mx-auto px-6">
           <div className="flex justify-between items-end mb-12">
             <div>
                <h2 className="text-3xl font-light text-white mb-2">Actualidad Cabo Vírgenes</h2>
              ...
              {NEWS_DATA.map((news) => (
                ...
                >
                  <div className="relative h-48 overflow-hidden">
                    <img src={news.img} className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-110" alt={news.titulo} />
                    <div className="absolute top-4 left-4 bg-[#0b1221]/90 backdrop-blur text-cyan-400 text-[10px] font-bold uppercase tracking-widest px-3 py-1 rounded-full border border-cyan-500/20">{news.medio}</div>
                  </div>
                  <div className="p-6">
                    ...
                    <h3 className="text-lg font-medium text-white group-hover:text-cyan-400 transition-colors mb-3 line-clamp-2">{news.titulo}</h3>
        ...
        <div className="container mx-auto px-6 relative z-10">
          ...
          <div className="grid lg:grid-cols-2 gap-16 mb-24 items-start" id="contacto">
            ...
            <div className="space-y-12">
               <div>
                 <span className="text-cyan-500 font-bold tracking-[0.2em] text-xs uppercase mb-4 block">Contacto</span>
                 <h2 className="text-5xl md:text-6xl font-light text-white mb-6 leading-tight">
                   Empecemos una <br/> <span className="font-serif italic text-slate-400">conversación.</span>
                 </h2>
               ...
               <div className="space-y-6">
                 <div className="flex items-center gap-6 p-4 rounded-2xl hover:bg-white/5 transition-colors group cursor-pointer border border-transparent hover:border-white/5">
                   ...
                   <div>
                     ...
                     <div className="text-white text-lg group-hover:text-cyan-400 transition-colors">sales@cabovirgenes.es</div>
                   </div>
                 </div>
               ...
               <form className="space-y-6">
                  <div className="grid grid-cols-2 gap-6">
                    <div className="space-y-2">
                      ...
                      <input type="text" className="w-full bg-[#050b14]/50 border border-white/10 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-cyan-500/50 focus:ring-1 focus:ring-cyan-500/50 transition-all" placeholder="Tu nombre" />
                    </div>
                    <div className="space-y-2">
                      <label className="text-xs font-bold text-slate-400 uppercase tracking-widest ml-1">Email</label>
                      <input type="email" className="w-full bg-[#050b14]/50 border border-white/10 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-cyan-500/50 focus:ring-1 focus:ring-cyan-500/50 transition-all" placeholder="tucorreo@empresa.com" />
                    </div>
                  </div>
                  <div className="space-y-2">
                    <label className="text-xs font-bold text-slate-400 uppercase tracking-widest ml-1">Mensaje</label>
                    <textarea rows="4" className="w-full bg-[#050b14]/50 border border-white/10 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-cyan-500/50 focus:ring-1 focus:ring-cyan-500/50 transition-all resize-none" placeholder="¿En qué podemos ayudarte?"></textarea>
                  </div>
                  <button className="w-full bg-gradient-to-r from-cyan-600 to-blue-600 text-white font-bold uppercase tracking-widest text-xs py-4 rounded-xl hover:shadow-[0_0_30px_rgba(6,182,212,0.4)] transition-all flex items-center justify-center gap-2 group">
                    Enviar Mensaje <ArrowRight className="w-4 h-4 group-hover:translate-x-1 transition-transform" />
                  </button>
               </form>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12 mb-16">
             ...
             <div className="space-y-6">
               <div className="flex items-center gap-3">
                 ...
                 <span className="font-bold tracking-[0.2em] text-lg uppercase text-white">Cabo Vírgenes</span>
               </div>
               ...
               <div className="flex gap-4">
                 {[Instagram, Linkedin, Facebook].map((Icon, i) => (
                   <a key={i} href="#" className="w-10 h-10 rounded-full bg-white/5 border border-white/5 flex items-center justify-center text-slate-400 hover:bg-cyan-500 hover:text-white hover:border-cyan-500 transition-all duration-300">
             ...
             <div>
               ...
               <p className="text-slate-500 text-sm mb-4">Suscríbete para recibir novedades de la industria.</p>
               <div className="flex bg-white/5 rounded-lg border border-white/10 p-1">
                 <input type="email" placeholder="Email" className="bg-transparent text-white px-3 py-2 text-sm w-full focus:outline-none placeholder:text-slate-600" />
      ...
      {isVideoModalOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/80 backdrop-blur-md p-4 animate-fade-in" onClick={() => setIsVideoModalOpen(false)}>
          <div 
            ...
            <div className="relative aspect-video bg-black group flex items-center justify-center">
              ...
              {videoSource ? (
                <>
                  ...
                  <div className="absolute bottom-0 left-0 right-0 p-6 bg-gradient-to-t from-black/90 via-black/50 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <div className="flex items-center gap-4">
                        <button onClick={changeSpeed} className="text-white hover:text-cyan-400 transition-colors flex items-center gap-1">
                          <Settings className="w-4 h-4" />
                          <span className="text-xs font-bold">{playbackSpeed}x</span>
                        </button>
                        <button 
                          onClick={() => setShowSubtitles(!showSubtitles)} 
                          className={`transition-colors ${showSubtitles ? 'text-cyan-400' : 'text-white hover:text-cyan-400'}`}
                        >
                ...
                <div className="flex flex-col items-center justify-center text-slate-500 p-10">
                  ...
                  <p className="text-sm uppercase tracking-widest font-bold text-slate-400">No hay video disponible</p>
                  <p className="text-xs text-slate-600 mt-2">El contenido multimedia no se encuentra accesible.</p>
                </div>
              )}
              ...
              <div className="flex items-start justify-between">
                <div>
                  <div className="flex items-center gap-3 mb-2">
                    <span className="text-cyan-400 text-[10px] font-bold uppercase tracking-widest border border-cyan-500/30 px-2 py-0.5 rounded">Institucional</span>
                    <span className="text-slate-500 text-[10px] font-bold uppercase tracking-widest">4K Ultra HD</span>
                  </div>
      ...
      {selectedShip && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-fade-in" onClick={() => setSelectedShip(null)}>
          <div className="glass-modal w-full max-w-2xl rounded-[2rem] overflow-hidden relative flex flex-col max-h-[85vh] shadow-2xl shadow-cyan-900/20" onClick={(e) => e.stopPropagation()}>
            <div className="relative h-48 shrink-0">
               ...
               <div className="absolute bottom-4 left-6"><span className="text-cyan-400 font-bold tracking-widest text-[10px] uppercase mb-1 block">{selectedShip.type}</span><h2 className="text-3xl font-light text-white">{selectedShip.name}</h2></div>
            </div>
            <div className="p-6 overflow-y-auto custom-scrollbar bg-gradient-to-b from-[#0b1221]/90 to-[#0f172a]/90 space-y-8">
               <div><h3 className="text-sm font-bold text-white mb-3 flex items-center gap-2 uppercase tracking-wider"><Navigation className="w-4 h-4 text-cyan-500" /> Especificaciones</h3>
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">{Object.entries(selectedShip.specs).map(([key, value]) => (<div key={key} className="bg-white/5 border border-white/10 rounded-xl p-3 text-center"><span className="text-[9px] text-slate-400 uppercase tracking-widest block mb-1">{key}</span><span className="text-white text-sm font-medium">{value}</span></div>))}</div>
               </div>
               <div><div className="flex justify-between items-center mb-3"><h3 className="text-sm font-bold text-white flex items-center gap-2 uppercase tracking-wider"><Activity className="w-4 h-4 text-green-500 animate-pulse" /> Telemetría</h3><span className="text-[9px] bg-green-500/20 text-green-400 px-2 py-0.5 rounded border border-green-500/30 font-bold">LIVE</span></div>
                  <div className="grid grid-cols-2 gap-3">
                     <div className="bg-[#0f172a] rounded-xl p-3 border border-cyan-500/30 flex items-center justify-between"><div><div className="text-[9px] text-slate-400 uppercase mb-1">Bodega</div><div className="text-lg font-bold text-white">{selectedShip.iot.temp_bodega}</div></div><Thermometer className="w-5 h-5 text-cyan-400" /></div>
                     <div className="bg-[#0f172a] rounded-xl p-3 border border-white/5 flex items-center justify-between"><div><div className="text-[9px] text-slate-400 uppercase mb-1">Velocidad</div><div className="text-lg font-bold text-white">{selectedShip.iot.velocidad}</div></div><Gauge className="w-5 h-5 text-slate-500" /></div>
                     <div className="bg-[#0f172a] rounded-xl p-3 border border-white/5 flex items-center justify-between"><div><div className="text-[9px] text-slate-400 uppercase mb-1">Rumbo</div><div className="text-lg font-bold text-white">{selectedShip.iot.rumbo}</div></div><Wind className="w-5 h-5 text-slate-500" /></div>
                     <div className="bg-[#0f172a] rounded-xl p-3 border border-white/5 flex items-center justify-between"><div><div className="text-[9px] text-slate-400 uppercase mb-1">Fuel</div><div className="text-lg font-bold text-white">{selectedShip.iot.combustible}</div></div><Fuel className="w-5 h-5 text-slate-500" /></div>
                  </div>
               </div>
               <div><h3 className="text-sm font-bold text-white mb-3 flex items-center gap-2 uppercase tracking-wider"><MapPin className="w-4 h-4 text-purple-500" /> Localización</h3><div className="bg-[#0f172a] rounded-xl border border-white/10 p-1 h-48 relative overflow-hidden"><img src="https://images.unsplash.com/photo-1451187580459-43490279c0fa?q=80&w=800" className="absolute inset-0 w-full h-full object-cover opacity-60 mix-blend-luminosity" alt="Map" /><div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 flex flex-col items-center"><div className="w-12 h-12 rounded-full border border-cyan-500/30 flex items-center justify-center animate-ping absolute"></div><div className="w-3 h-3 bg-cyan-500 rounded-full shadow-[0_0_15px_#06b6d4] relative z-10 border-2 border-white"></div></div><div className="absolute bottom-2 left-2 bg-black/70 backdrop-blur px-2 py-1 rounded text-[9px] text-cyan-400 font-mono border border-white/10">{selectedShip.coordinates}</div></div></div>
            </div>
          </div>
        </div>
      )}
      ...
      {selectedProduct && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-fade-in" onClick={() => setSelectedProduct(null)}>
          <div className="glass-modal w-full max-w-2xl rounded-[2rem] overflow-hidden relative flex flex-col max-h-[85vh] shadow-2xl shadow-cyan-900/20" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="relative h-64 shrink-0">
               ...
               <div className="absolute inset-0 bg-gradient-to-t from-[#0b1221] to-transparent"></div>
               <button onClick={() => setSelectedProduct(null)} className="absolute top-4 right-4 w-8 h-8 rounded-full bg-black/40 backdrop-blur text-white flex items-center justify-center hover:bg-white hover:text-black transition-all border border-white/10"><X className="w-4 h-4" /></button>
               <div className="absolute bottom-6 left-8">
                 <div className="flex gap-2 mb-2">
                   <span className="bg-cyan-500/20 border border-cyan-500/30 text-cyan-300 px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-widest">{selectedProduct.tag}</span>
                   {selectedProduct.subtitle && <span className="bg-white/10 border border-white/10 text-white px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-widest">{selectedProduct.subtitle}</span>}
                 </div>
                 <h2 className="text-3xl md:text-4xl font-light text-white leading-none">{selectedProduct.title}</h2>
               </div>
            </div>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    {Object.entries(selectedProduct.details).map(([key, value]) => (
                      <div key={key} className="bg-white/5 p-3 rounded-xl border border-white/5">
                        <span className="text-[9px] text-slate-400 uppercase tracking-widest block mb-1">{key.replace('_', ' ')}</span>
               ...
               <div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {selectedProduct.formatos.map((fmt, i) => (
                      <span key={i} className="px-3 py-1.5 rounded-lg bg-[#0f172a] border border-white/10 text-xs text-slate-300 flex items-center gap-2">
            ...
            <form className="space-y-4">
              ...
              <div className="pt-4 flex flex-col gap-3">
                <button className="w-full bg-gradient-to-r from-cyan-600 to-blue-600 text-white font-bold uppercase tracking-widest text-xs py-3.5 rounded-xl hover:shadow-[0_0_20px_rgba(6,182,212,0.3)] transition-all flex items-center justify-center gap-2 group">
                  Enviar Correo <Mail className="w-4 h-4 group-hover:translate-x-1 transition-transform" />
                </button>
