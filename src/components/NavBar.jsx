...
import { Anchor, ArrowRight, ChevronDown, Menu, X } from 'lucide-react';
              ...
              >
                  ...
                  {item.subitems && item.subitems.length > 0 && (
                    <ChevronDown
                      className={`w-3 h-3 transition-transform duration-300 ${
                        activeSubmenu === item.id ? 'rotate-180 text-cyan-400' : ''
                      }`}
                    />
                  )}
                </a>
                <span
                  className={`absolute bottom-0 left-1/2 -translate-x-1/2 h-[2px] bg-cyan-400 transition-all duration-300 ${
                    activeSubmenu === item.id ? 'w-full' : 'w-0 group-hover:w-full'
                  }`}
                ></span>
              </div>
            ))}
          ...
          <div className="glass-submenu rounded-2xl p-8 grid grid-cols-12 gap-8">
            <div className="col-span-4 border-r border-white/10 pr-6">
              <h3 className="text-2xl font-light text-white mb-2">
                {navItems.find((i) => i.id === activeSubmenu)?.label}
              </h3>
